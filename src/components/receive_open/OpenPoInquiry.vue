                ...
                <div class="table-responsive">
                    <table class="table table-bordered">
                        <thead >
                            ...
                            <tr>
                                <th colspan="2">
                                    <button type="button" :disabled="btn_search" class="btn btn-sm btn-primary col-xs-12" v-on:click="fetch_open_inquiry()">
            ...
            <div class="panel panel-flat">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <tr v-for="(row, i) in list_open_inquiry" :key="i">
                                <td>{{ row.date_ | date }}</td>
                                <td style="text-align:center;font-size:12px;">
                                    PO # {{ row.open_id }}<br>
                                    INVOICE #: {{ row.inv_no }} <br>
                                    RECEIVING NO #: {{ row.receiving_no }} <br>
                                    SUPPLIER #: {{ row.supplier_name }} <br>
                                    POSTED BY #: {{ row.user_id }} <br>
                                    NET TOTAL #: {{ row.net_total }} <br>
                                    STATUS: {{ row.posted }} <br>
                                </td>
                            </tr>
                        </tbody>
                    </table>
...
export default {
    ...
    async mounted(){
        getSessionCurrentUrl(this, 'OpenPoInquiry')
        await this.fetch_open_inquiry()
    },
    methods:{
        async fetch_open_inquiry(){
            ...
            try {
                ...
                let result  = await axios.get(apiUrl('purchaser/receive_po/op/inquiry/fetch_open_inquiry'), query)
                let data    = result.data
                this.list_open_inquiry = data.list_open_po
            } catch (error) {
