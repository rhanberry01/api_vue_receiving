...
export default {
    ...
    async mounted(){
        ...
        getSessionCurrentUrl(this, 'TransferCommunityPalengkeReceive')
    },
    methods: {
        async fetch_receive_transfer(page=1) {
            ...
            try {
                ...
                let result  = await axios.get(apiUrl('purchaser/receive_po/transfer-community-palengke/receive/fetch_receive_transfer'), query)
        ...
        async view_receive(transfer_id) {
            this.$router.push({ name: 'TransferCommunityPalengkeReceiveView', query: { transfer_id } })
        },
        async receive_receive(transfer_id) {
            this.$router.push({ name: 'TransferCommunityPalengkeReceiveReceive', query: { transfer_id } })
        },
