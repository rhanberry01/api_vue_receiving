    ...
    methods:{
        async fetch_transfer_slip(){
            ...
            try {
                ...
                let result  = await axios.get(apiUrl('purchaser/receive_po/transfer-community-palengke/inquiry/fetch_transfer_slip'), query)
        ...
        async view_transfer_slip_item(transfer_id) {
            try {
                this.$router.push({ name: 'TransferCommunityPalengkeInquirySlipItem', query: { transfer_id } })
            } catch (error) {
                await this.handdleError(error)
            }
        },
