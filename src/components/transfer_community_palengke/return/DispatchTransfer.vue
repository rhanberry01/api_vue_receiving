                    ...
                    <table class="table table-bordered">
                        <thead>
                            ...
                            <tr v-show="!loading_dispatch" v-for="(row, i) in list_dispatch_transfer" :key="i">
                                ...
                                <a v-on:click="receive_dispatch(row.id)">RETURN</a></th>
                            </tr>
                            <tr>
...
export default {
    ...
    async mounted(){
        ...
        getSessionCurrentUrl(this, 'TransferCommunityPalengkeReturn')
    },
    methods: {
        async fetch_dispatch_transfer(page=1) {
            ...
            try {
                ...
                let result  = await axios.get(apiUrl('purchaser/receive_po/transfer-community-palengke/return/fetch_dispatch_transfer'), query)
        ...
        async view_dispatch(transfer_id) {
            this.$router.push({ name: 'TransferCommunityPalengkeReturnView', query: { transfer_id } })
        },
        async receive_dispatch(transfer_id) {
            this.$router.push({ name: 'TransferCommunityPalengkeReturnReceive', query: { transfer_id } })
        },
