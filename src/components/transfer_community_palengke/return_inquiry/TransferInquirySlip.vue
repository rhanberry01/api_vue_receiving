                    ...
                    <table class="table table-bordered">
                        <thead>
                            <tr v-for="(row, i) in list_transfer_slip" :key="i">
                                <th style="text-align:center;font-size:12px;">
                                    DATE CREATED #: {{ row.date_ | date }}<br>
                                    SEQ #: {{ row.movement_no }} <br>
    ...
    methods:{
        async fetch_transfer_slip(){
            ...
            try {
                ...
                let result  = await axios.get(apiUrl('purchaser/receive_po/transfer-community-palengke/inquiry/return/fetch_transfer_slip'), query)
        ...
        async view_transfer_slip_item(transfer_id) {
            try {
                this.$router.push({ name: 'TransferCommunityPalengkeInquiryReturnItem', query: { transfer_id } })
            } catch (error) {
                await this.handdleError(error)
            }
        },
